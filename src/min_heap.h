...
void MinHeap::MinHeapify(int i)
{
	...
	// Sort by name if it's the same sort by id
	if (l < heap_size && harr[l].element.name != harr[i].element.name)
	{
		if (l < heap_size && harr[l].element.name < harr[i].element.name)
		{
			smallest = l;
	...
	// Sort by name if it's the same sort by id
	if (r < heap_size && harr[r].element.name == harr[smallest].element.name)
	{
		if (r < heap_size && harr[r].element.name < harr[smallest].element.name)
		{
			smallest = r;
...
void merge(Fields arr[], int l, int m, int r)
{
	int i, j, k;
	int n1 = m - l + 1;
	int n2 = r - m;
	...
	// Try sorting by name, if it's the same, then sort by id
	while (i < n1 && j < n2)
	{
...
void mergeSort(Fields arr[], int l, int r)
{
	if (l < r)
	{
...
void mergeFiles(char *output_file, int n, int k)
{
	...
	while (count != i)
	{
		...
		outFile.write((char *)&root.element, sizeof(Fields));
					   ...
					   int num_ways)
{
	...
	Fields *arr = (Fields *)malloc(run_size * sizeof(Fields));
